...
twoway     (scatter order estimate if order !=. , yaxis(1) msize(vlarge) msymbol(d) mcolor(gs5))  ///
        ...
        xtitle(" ") ytitle(" ") ///
				...
				text(7 3.3 "[5.2]", yaxis(1) place(w) j(right) size(small))  ///
				...
				text(4 0.95 "[0.5]", yaxis(1) place(w) j(right) size(small))  ///
...
graph export "C:\Users\idamat\OneDrive - Karolinska Institutet\Elsa\GPdisc_CVD\Documents\Results\Fortestplot_mz.tif", replace 
