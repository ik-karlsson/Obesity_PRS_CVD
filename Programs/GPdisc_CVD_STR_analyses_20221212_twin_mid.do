...
cap postclose coxreg_cvd
	postfile coxreg_cvd str20  (PRScat BMIall_ow BMItwin_ow BMIdz_ow BMImz_ow BMIall_ob BMItwin_ob BMIdz_ob BMImz_ob) ///
								using P:\Dementia_IK\SfoEpi\GPdisc_CVD\Output\GOSH\GPdisc_CVD_STR_analyses_20221212_twin_mid_CVDany.dta, replace
	set more off
	// BMI, categorical, total sample
	stcox i.BMImid_cat educ sex smoke i.studyn, vce(cluster pairid)
		lincom 2.BMImid_cat
	...
	set more off
	// BMI, categorical, total sample
	stcox i.BMImid_cat educ sex smoke i.studyn if(tert_PRS_BMI==`j'), vce(cluster pairid)
		lincom 2.BMImid_cat
		local m_BMIall_ow = r(estimate)
		local se_BMIall_ow = r(se)
		local Lci_BMIall_ow = exp(`m_BMIall_ow'-1.96*`se_BMIall_ow')
...
translate session.smcl "P:\Dementia_IK\SfoEpi\GPdisc_CVD\Log\GOSH\GPdisc_CVD_STR_analyses_20221212_twin_mid.log", replace
